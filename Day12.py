...
class Record:
    def __init__(self, springs: str, groups_to_find: List[int]):
        ...
        print(f"New Record {self.groups_to_find}\n")
    ...
    def find_matches(self, string: str):
        ...
        unknown_groups = [group for group in groups if '#' not in group]
        if len(definite_groups) > len(self.groups_to_find):
            # or \
            #     len(definite_groups) + len(unknown_groups) < len(self.groups_to_find):
            return []
